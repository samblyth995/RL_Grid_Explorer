...
import numpy as np
from Agent_QNet import QNetwork
...
q_network = QNetwork(input_size=state_size, hidden_size=hidden_size, output_size=action_size)
target_network = QNetwork(input_size=state_size, hidden_size=hidden_size, output_size=action_size)
...
##write runs to csv
with open('training_runs_optim_QNET_6_X_6.csv', mode='w', newline='') as file:
...
for episode in range(NUM_EPISODES):
    ...
    #     break
# print info to csv
    with open('training_runs_optim_QNET_6_X_6.csv', mode='a', newline='') as file:
        writer = csv.writer(file)
        final_position= env.agent_position
